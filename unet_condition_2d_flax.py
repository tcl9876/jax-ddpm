...
class FlaxUNet2DConditionModel(nn.Module, FlaxModelMixin, ConfigMixin):
    ...
    mid_block_type: str = "FlaxUNetMidBlock2DCrossAttn",
    ...
    def setup(self):
        ...
        # mid. we deviate from the 
        if self.mid_block_type == "CrossAttnMidBlock2D":
            ...
            self.mid_block = FlaxResnetBlock2D(
                in_channels=block_out_channels[-1],
    ...
    def get_eps_from_x(self, original_sample, model_output, timesteps):
        #formula for epsilon given xstart and xt is:
        #eps = (xt - sqrt(alpha_cumprod)*xstart) / sqrt(1 - alpha_cumprod)
        if self.alphas_cumprod is None:
            raise Exception("self.alphas_cumprod cannot be None when using an output type other than eps")
        
        alphas_cumprod = self.alphas_cumprod[timesteps]
        pred_eps = (original_sample - jnp.sqrt(alphas_cumprod) * model_output) / jnp.sqrt(1 - alphas_cumprod)
        return pred_eps


    def get_eps_from_v(self, original_sample, model_output, timesteps):
        #https://arxiv.org/pdf/2210.02303.pdf found using v-prediction was beneficial for video generation,
        #so we include it here as an option for output parameterization
        #formula for xstart given v and xt is:
        #x = sqrt(alpha_cumprod) * xt - sqrt(1 - alpha_cumprod) * model_output
        #from this we can get epsilon
        if self.alphas_cumprod is None:
            raise Exception("self.alphas_cumprod cannot be None when using an output type other than eps")

        alphas_cumprod = self.alphas_cumprod[timesteps]
        pred_x = original_sample * jnp.sqrt(alphas_cumprod)  -  jnp.sqrt(1 - alphas_cumprod) * model_output
        pred_eps = (original_sample - jnp.sqrt(alphas_cumprod) * pred_x) / jnp.sqrt(1 - alphas_cumprod)
        return pred_eps
    ...
    def __call__(
        ...
        Args:
            ...
            scheduler (`jnp.ndarray` or `None`, *optional*, defaults to `None`):
        ...
        # 4. mid
        if self.mid_block_type == "FlaxUNetMidBlock2DCrossAttn":
        ...
        #enable the model to use an output type other than epsilon-prediction
        #this is converted back into epsilon for compatibility with other parts of the codebase, e.g. the schedulers
        if self.output_type.lower() == 'v':
            sample = self.get_eps_from_v(sample, timesteps)
        if self.output_type.lower() == 'x':
            sample = self.get_eps_from_x(sample, timesteps)
